...
#CAP 4603 _ Dr. Marques _ 6/16/2022
#Project 1
#Elizabeth Jijon
#Peyton Adkins
#_____________________________________________________________________________________________________
...
class Checker(TwoPlayerGame):
    ...
    def make_move(self, pos):
        ...
        #set position of current player's pieces to the appropriate piece positions from pos

    def lose(self):
        for (x,y) in self.players[1].pos: #for all sets of coordinates among black pieces
            if (x,y) in self.white_territory: #check for matching coordinates in white territory
                return 1 #if so, game has concluded
        for (x,y) in self.players[0].pos: #try again with white pieces in black territory
            if (x,y) in self.black_territory:
                return 1
        return 0 #if neither test returns, return 0
            

    def is_over(self):
        return (self.possible_moves() == []) or self.lose()
        #game ends if there are no more possible moves or lose condition returns true
    ...
    def scoring(self):
        ...
        #return -100 if the game has been lost otherwise 0
